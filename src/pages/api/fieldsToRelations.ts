...
export default function fieldsToRelations<T>(
...
): FindOptions<T, string> {
	const paths: string[][] = [];
	
	function nested( field: any, key: string = undefined, deep = 0, parent: string[] = [] ) {
		if ( Object.values( field ).length === 0 ) return;
		...
		Object.keys( field ).forEach( ( key: any ) => { // TODO: check if can be switch to exclude id
	...
	const value = !options.root
		...
		: options.root.split( '.' ).reduce( ( p, prop ) => p[ prop ],
			graphqFields( info, {}, { excludedFields: options.excludeFields } ) );
	
	nested( value, options.root ? options.root.split( '.' ).pop() : undefined );
	
	return { populate: paths.map( ( list: string[] ) => list.join( '.' ) ) as any };
}
